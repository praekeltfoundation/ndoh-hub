...
class Command(BaseCommand):
    ...
    help = (
        "Make HTTP requests to a ndoh-hub instance to initiate opt-out "
        "Changes for MSISDNs read from a CSV file (expected format "
        "<id>;<msisdn>;<count>)"
    )

    def add_arguments(self, parser):
        parser.add_argument("--csv", type=str, help="CSV file to parse for MSISDNs")
        parser.add_argument(
            "--hub-token",
            type=str,
        ...
        parser.add_argument(
            ...
            default=False,
            help=(
                "Don't use the Identity Store API, expects CSV with inline "
                "Identity UUIDs"
            ),
        )
        parser.add_argument(
        ...
        parser.add_argument("--start", type=int, help="The row number to start from")
        parser.add_argument(
            "--end",
            type=int,
    ...
    def handle(self, *args, **options):
        ...
        if not no_is:
            ...
            if not identity_store_token:
                raise CommandError("--identity-store-token is a required " "parameter")
        ...
        with open(file_name) as csv_file:
            ...
            if start and end:
                ...
                raise CommandError(
                    "--start is a required parameter when " "specifying --end"
                )
            else:
                rows = csv_reader
                row_count = 1
            ...
            for idx, row in enumerate(rows, start=row_count):
                ...
                if no_is:
                    ...
                    result = list(
                        ids_client.get_identity_by_address("msisdn", msisdn)["result"]
                    )
                    if len(result) < 1:
                        loop_log.error("Could not load identity for msisdn.")
                        continue
                ...
                try:
                    ...
                    id_log.error("Connection error to Hub API: {}".format(exc.message))
                    break
