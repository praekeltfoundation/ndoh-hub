...
# from random_contacts import get_contacts
import random_contacts
import responses


@override_settings(TURN_URL="https://turn", TURN_TOKEN="turn_token")
class HandleRapidProContactsTests(TestCase):
    ...
    def test_get_contacts(self):
        responses.add(
            ...
            json=[{'uuid': '148947f5-a3b6-4b6b-9e9b-25058b1b7800',
...
class HandleTurnProfileTests(TestCase):
    ...
    def test_get_turn_profile_link(self):
        responses.add(
            ...
            json={'chat': {'assigned_to': None, 'owner': '+27836378500',
                           'permalink': 'https://app.turn.io/c/68cc14b3-6a4e-4962-82ed-c572c6836fdd',
                           'state': 'OPEN', 'state_reason': 'Re-opened by inbound message.',
                           'unread_count': 0, 'uuid': '68cc14b3-6a4e-4962-82ed-c572c6836fdd'}},
            status=200,
        )
        response = random_contacts.get_turn_profile_link('27781234567')
        print(str(response))
        self.assertEqual(type(response), str)
        self.assertEqual(str(response), 'https://app.turn.io/c/68cc14b3-6a4e-4962-82ed-c572c6836fdd')
        self.assertNotEqual(type(response), 'https://app.turn.io/c/68cc14b3-6a4e-4962-82ed-c572c6836fdz')

    @responses.activate
    def test_get_turn_profile_link_none_contact(self):
        # Test if sent none contact to return no value
        contact = None
        responses.add(
            responses.GET,
            ...
            json={'chat': {'assigned_to': None, 'owner': '+27836378500',
                           ...
                           'state': 'OPEN', 'state_reason': 'Re-opened by inbound message.',
                           'unread_count': 0, 'uuid': '68cc14b3-6a4e-4962-82ed-c572c6836fdc'}},
            status=200,
        )
...
class HandleSendSlackMessage(TestCase):
    def setUp(self):
        self.contact_details = [{'Rapid_Pro_Link: ':
                            'http://rapidpro.qa.momconnect.co.za/contact/read/d62aabca-7b6c-4d32-a017-1a30a8a27663/',
