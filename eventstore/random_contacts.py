...
if settings.EXTERNAL_REGISTRATIONS_V2:
    ...
    if settings.RAPIDPRO_URL and settings.RAPIDPRO_TOKEN:
        # url = urljoin(settings.RAPIDPRO_URL, '/api/v2/contacts.json')
...
def get_contacts():
    ...
    contact_details = []
    try:
        for contact_batch in rapidpro.get_contacts(before=before, after=after).iterfetches(retry_on_rate_exceed=True):
            for contact in contact_batch:
                contact_uuid = contact.uuid
                contact_urns = contact.urns

                if contact_uuid and contact_urns:
                    whatsapp_number = [i.split(':')[1] for i in contact_urns if 'whatsapp' in i]
...
def send_slack_message(contact_details):
    print('Contacts: ****', settings.SLACK_URL, settings.SLACK_TOKEN)
    # Send message to slack
    try:
        response = requests.post(urljoin(settings.SLACK_URL, '/api/chat.postMessage'),
                      {
