...
class TokenAuthMiddlewareTests(TestCase):
    def test_user_exists(self):
        ...
        scope = {"query_string": urlencode({"token": str(token.key)}).encode()}
    ...
    def test_no_token(self):
        ...
        """
        scope = {"query_string": urlencode({"token": "bad-token"}).encode()}
        mw = TokenAuthMiddleware(lambda _: None)
        mw(scope)
    ...
    def test_blank_token_provided(self):
        ...
        """
        scope = {"query_string": urlencode({"token": ""}).encode()}
        mw = TokenAuthMiddleware(lambda _: None)
        mw(scope)
        self.assertEqual(scope["user"], None)
