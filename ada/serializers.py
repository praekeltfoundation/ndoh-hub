...
class AdaChoiceTypeSerializer(serializers.Serializer):
    def validate_value(self, data):
        ...
        if not (0 <= user_input <= choices):
            message = f"Something seems to have gone wrong. You entered {user_input} but there are only {choices} options."
            data["error"] = message
            raise serializers.ValidationError(data)
        return data


class AdaTandCTypeSerializer(serializers.Serializer):
    def validate_value(self, data):
        user_input = data["value"]
        if user_input != "1" or user_input != "accept" or user_input != "continue":
